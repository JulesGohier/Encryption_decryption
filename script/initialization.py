...
def input_file(file_name: str):
    ...
    """
    with open("./Messages_to_encrypt/" + file_name, "r", encoding="utf-8") as file:
        text: str = file.read()
    return text
...
def output_file(text: str):
    ...
    with open("./Messages_to_encrypt/" + file_name, "w", encoding="utf-8") as file:
        file.write(text)
...
def choice_output(text: str):
    ...
    """
    choice = input(
        "Do you want to: display the message (D), save it to a file (S) or both (DS) ? "
    )
    if choice == "S":
        output_file(text)
    elif choice == "D":
...
def choice_code_break_caesar(text: str):
    """
    Function to select the method of breaking the Caesar code: by frequency of appearance,
    by brute force or by the decryption key
    :param text: Number of characters in the text because a minimum for the frequency of appearance
    :return: Returns the choice F for frequency of appearance,
    B for brute force and K for the decryption key
    """
    nb_character = len(text)
    choice: str = input(
        "Do you want to: break the cesar code by brute force (B),"
        " by frequency of appearance of the letters (F) "
        "or with the decryption key (K) ? "
    )
    while choice not in ("B", "F", "K"):
        choice = input("Choose between B, F, K : ")

    if nb_character < 185 and choice == "F":
        print(
            "The text to be decrypted does not contain enough characters to break "
            "the cesar code by frequency of "
            "occurrence"
        )
        while choice not in ("B", "K"):
...
def creation_dictionary_json(dict_lowercase_uppercase: dict):
    ...
    for i in range(52):
        dict_lowercase_uppercase[
            list_lowercase_uppercase[i]
        ] = list_lowercase_uppercase_shuffle[i]

    file_name: str = input("Enter the name of the json dictionary : ")
    with open(
        "./Messages_to_encrypt/" + file_name + ".json", "w", encoding="utf-8"
    ) as dictionary:
...
def open_json_dictionary(file_name: str):
    ...
    """
    with open(
        "./Messages_to_encrypt/" + file_name + ".json", "r", encoding="utf-8"
    ) as dictionary:
...
def reverse_json_dictionary(dict_original: dict):
    ...
    :param dict_original:  that we want to reverse
    :return: Return the inverted dictionary
    """
    dict_reverse: dict = {}
